...
import { ArrowUp, Layers, ScrollText } from 'lucide-react';
          ...
          <div className="flex items-center gap-2">
            {onToggleViewMode && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/[0.03] dark:bg-white/[0.05] border border-black/[0.08] dark:border-white/[0.1] text-xs font-manrope text-[#1A1A1A] dark:text-[#E0E0E0] hover:border-[#FF5600]/40 transition-all cursor-pointer shadow-xs"
              >
                {viewMode === 'panel' ? <Layers className="w-3.5 h-3.5 text-[#FF5600]" /> : <ScrollText className="w-3.5 h-3.5 text-[#FF5600]" />}
                <span>Toggle: {viewMode === 'panel' ? 'Slide Deck' : 'Scroll Page'}</span>
              </button>
            )}
