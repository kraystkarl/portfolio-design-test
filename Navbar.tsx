...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navItems = [
    ...
    { label: 'Proof of Work', index: 4, kicker: '04' },
    { label: 'Evidence Pack', index: 5, kicker: '05' },
    { label: 'Contact', index: 6, kicker: '06' },
  ];

  return (
    <header className="fixed top-2.5 sm:top-4 inset-x-0 z-50 px-3 sm:px-6 select-none pointer-events-none">
      <div className="max-w-7xl mx-auto flex items-center justify-between pointer-events-auto">
        ...
        {/* Right: Mode Toggle + Appearance + Resume + Mobile Hamburger */}
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/[0.05] dark:bg-white/[0.08] border border-black/10 dark:border-white/12 text-xs font-manrope font-medium text-[#1A1A1A] dark:text-[#E0E0E0] hover:border-[#FF5600]/60 active:scale-95 transition-all cursor-pointer backdrop-blur-md"
            title={`Current: ${viewMode === 'panel' ? 'Cinematic Slide Panels' : 'Continuous Scroll'}. Click to toggle view mode.`}
          >
            {viewMode === 'panel' ? (
              <>
                <Layers className="w-3.5 h-3.5 text-[#FF5600]" />
                <span className="hidden md:inline">Slide Mode</span>
              </>
            ) : (
              <>
                <ScrollText className="w-3.5 h-3.5 text-[#FF5600]" />
                <span className="hidden md:inline">Scroll Mode</span>
              </>
            )}
          </button>
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="p-2 rounded-full bg-black/[0.05] dark:bg-white/[0.08] border border-black/10 dark:border-white/12 text-[#1A1A1A] dark:text-[#E0E0E0] hover:bg-black/10 dark:hover:bg-white/15 active:scale-90 transition-all cursor-pointer backdrop-blur-md"
            aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
          >
            {theme === 'dark' ? (
              <Sun className="w-3.5 h-3.5 text-[#FF5600] transition-transform duration-300 hover:rotate-45" />
            ) : (
              <Moon className="w-3.5 h-3.5 text-[#1A1A1A] transition-transform duration-300 hover:-rotate-12" />
            )}
          </button>
          ...
          {onOpenBooking && (
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/[0.05] dark:bg-white/[0.08] border border-black/10 dark:border-white/12 text-xs font-manrope font-medium text-[#1A1A1A] dark:text-[#E0E0E0] hover:border-[#FF5600]/60 hover:text-[#FF5600] active:scale-95 transition-all cursor-pointer backdrop-blur-md"
            >
          ...
          <a
            ...
            download
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[#FF5600] text-white text-xs font-manrope font-bold hover:bg-[#E04C00] active:scale-95 transition-all shadow-[0_2px_14px_rgba(255,86,0,0.35)]"
